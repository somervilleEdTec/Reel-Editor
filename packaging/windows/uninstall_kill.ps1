# Stop Reelwrite processes so the uninstaller can delete files in use.
# Usage: powershell -ExecutionPolicy Bypass -File uninstall_kill.ps1 -InstallDir "C:\path\to\Reelwrite"
...
function Test-OwnedPid([int]$ProcessId, [string]$Root) {
  ...
  if ($name -notin @("Reelwrite", "reelwrite-api")) { return $false }
...
$pidFile = Join-Path $env:LOCALAPPDATA "Reelwrite\reelwrite.pid"
$apiPidFile = Join-Path $env:LOCALAPPDATA "Reelwrite\api.pid"
...
if ($root) {
  $prefix = $root.TrimEnd('\', '/') + [IO.Path]::DirectorySeparatorChar
  foreach ($name in @("Reelwrite", "reelwrite-api")) {
