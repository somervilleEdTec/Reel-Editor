...
from reelwrite import process_lifecycle as lifecycle
from reelwrite.pid_file import clear_pid_file, pid_file_path, read_pid_file, write_pid_file
from reelwrite.process_scan import ProcessInfo
...
def test_pid_file_write_read_clear(tmp_path: Path, monkeypatch):
    ...
    assert path == pid_file_path() == tmp_path / "reelwrite.pid"
...
def test_kill_targets_app_and_install_scoped_ffmpeg(tmp_path: Path, monkeypatch):
    ...
    procs = [
        _proc(99, "reelwrite-api.exe", str(install / "reelwrite-api.exe")),
        _proc(101, "Reelwrite.exe", str(install / "Reelwrite.exe")),
        _proc(102, "reelwrite-api.exe", str(install / "reelwrite-api.exe")),
    ...
    write_pid_file(200, "reelwrite")  # foreign pid must be ignored
    killed = lifecycle.kill_reelwrite_processes(str(install))
...
def test_kill_is_a_noop_when_nothing_matches(tmp_path: Path, monkeypatch):
    ...
    assert lifecycle.kill_reelwrite_processes(str(tmp_path)) == []
